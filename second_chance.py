...
def calc_grades(initial, second_chance, students):
    i = 0
    final_grades = []
    while i < students:
        if second_chance[i] == 10:
            temp = initial[i] + 2
            if temp > 10:
                temp = 10
        else:
            temp = initial[i]
        final_grades.append(temp)
        i += 1
    return final_grades


def show_grades(initial, final, students):
    i = 0
    change = 0
    while i < students:
        if initial[i] != final[i]:
            change += 1
        i += 1
    print(f'NOTAS ALTERADAS: {change}')

    j = 0
    while j < students:
        if initial[j] != final[j]:
            print(
                f'*(00{j+1}) original: ' + f'{initial[j]:.2f}'.zfill(5) + ' | final: ' + f'{final[j]:.2f}'.zfill(5))
        else:
            print(
                f'-(00{j+1}) original: ' + f'{initial[j]:.2f}'.zfill(5) + ' | final: ' + f'{final[j]:.2f}'.zfill(5))
        j += 1
...
initial_grades = students_grades(students)
second_chance = students_grades(students)

final_grades = calc_grades(initial_grades, second_chance, students)
show_grades(initial_grades, final_grades, students)
